    ...
    plugins: [
        {
            rules: {
                'require-scope-for-features': parsed => {
                    ...
                    if (type === 'feat' && !scope) {
                        return [false, 'Feature commits must include a scope to indicate which module is affected'];
                    }
                    return [true];
                },
                'require-breaking-change-footer': parsed => {
                    ...
                    if (hasBreakingInSubject && !hasBreakingInFooter) {
                        return [false, 'Breaking changes must include "BREAKING CHANGE:" in the footer with details'];
                    }
                    return [true];
                },
            },
